...
// Reuse the current Bun executable so `bun run dev` does not depend on PATH
// containing `bun` for the child process spawn on Windows.
const bunExecutable = process.execPath || "bun";

const child = Bun.spawn(
    [bunExecutable, ...inspectArgs, "run", ...defineArgs, ...featureArgs, cliPath, ...process.argv.slice(2)],
