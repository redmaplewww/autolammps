 ...
 * with the claude.ai OAuth token). isWebAppSubscriber() excludes
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    ...
    if (!isWebAppSubscriber()) {
      return 'Remote Control requires a ANGSHENG subscription. Run `angsheng auth login` to sign in with your ANGSHENG account.'
    }
    if (!hasProfileScope()) {
...
// program, before enableConfigs() runs. isWebAppSubscriber() → getGlobalConfig()
...
function isWebAppSubscriber(): boolean {
  try {
    return authModule.isWebAppSubscriber()
  } catch {
    return false
  }
}
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Angsheng (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`claude update\` to update.`
    }
  }
  return null
}
