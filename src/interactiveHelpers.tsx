...
import {
  ...
  getMemoryFiles,
  shouldShowMemoryPromptExternalIncludesWarning,
...
export async function showSetupScreens(
  ...
  // and checks ANGSHENG.md external includes. bypassPermissions mode
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (await shouldShowMemoryPromptExternalIncludesWarning()) {
      const externalIncludes = getExternalMemoryPromptIncludes(
        await getMemoryFiles(true),
      )
      const { MemoryPromptExternalIncludesDialog } = await import(
        './components/MemoryExternalIncludesDialog.js'
      )
      await showSetupDialog(root, done => (
  ...
  // processes but ignored by Angsheng itself (see auth.ts).
  ...
  // Show Chrome onboarding for first-time Angsheng in Chrome users
  if (
    claudeInChrome &&
    !getGlobalConfig().hasCompletedAngshengInChromeOnboarding
  ) {
    const { AngshengInChromeOnboarding } = await import(
      './components/AngshengInChromeOnboarding.js'
    )
    await showSetupDialog(root, done => (
      <AngshengInChromeOnboarding onDone={done} />
    ))
  }
