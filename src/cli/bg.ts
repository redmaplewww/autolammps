...
function getSessionsDir(): string {
  return join(getAppConfigHomeDir(), 'sessions')
}
...
/**
 * `aura daemon status` / `aura ps` — list live sessions.
 */
...
/**
 * `aura daemon logs <target>` — show logs for a session.
 */
...
/**
 * `aura daemon attach <target>` — attach to a background session.
 *
...
export async function attachHandler(target: string | undefined): Promise<void> {
  ...
  if (!target) {
    ...
    if (bgSessions.length === 0) {
      console.log(
        'No background sessions to attach to. Start one with `aura daemon bg`.',
      )
      return
    }
...
/**
 * `aura daemon kill <target>` — kill a session.
 */
...
/**
 * `aura daemon bg [args]` — start a background session.
 *
  ...
  if (
    ...
    console.error(
      'Error: Background sessions with detached engine require -p/--print flag.\n' +
        ...
        'Usage:\n' +
        '  aura daemon bg -p "your prompt here"\n' +
        '  echo "prompt" | aura daemon bg --pipe',
    )
