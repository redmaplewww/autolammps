/**
 * `aura update` — Check and install the latest version of agent-aura-cli.
 *
 * Detection strategy:
...
const PACKAGE_NAME = 'agent-aura-cli'
...
export async function updateCCB(): Promise<void> {
  ...
  if (latestVersion === currentVersion || gte(currentVersion, latestVersion)) {
    writeToStdout(chalk.green(`aura is up to date (${currentVersion})`) + '\n')
