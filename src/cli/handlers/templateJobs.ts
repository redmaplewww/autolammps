...
Template Job Commands:
  ...
  aura job list                    List available templates
  aura job new <template> [args]   Create a new job from a template
  aura job reply <job-id> <text>   Reply to an existing job
  aura job status <job-id>         Show job status
`)
}
...
function handleStatus(args: string[]): void {
  const jobId = args[0]
  if (!jobId) {
    console.error('Usage: aura job status <job-id>')
    process.exitCode = 1
    return
  }
...
function handleList(): void {
  ...
  if (templates.length === 0) {
    ...
    console.log('Place .md files in .angsheng/templates/ or ~/.angsheng/templates/')
    return
  }
...
function handleNew(args: string[]): void {
  ...
  if (!templateName) {
    console.error('Usage: aura job new <template> [args...]')
    process.exitCode = 1
    return
  }
...
function handleReply(args: string[]): void {
  ...
  if (!jobId || !text) {
    console.error('Usage: aura job reply <job-id> <text>')
    process.exitCode = 1
    return
  }
