...
export function MCPRemoteServerMenu({
  ...
  const handleClaudeAIAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        onComplete?.(
          'Authentication successful, but server still requires authentication. You may need to manually restart Agent Aura.',
        )
      } else {
        onComplete?.(
          'Authentication successful, but server reconnection failed. You may need to manually restart Agent Aura for the changes to take effect.',
        )
      }
    } catch (err) {
  ...
  const handleClaudeAIAuth = React.useCallback(async () => {
    const webAppBaseUrl = getOauthConfig().CLAUDE_AI_ORIGIN
    ...
    if (
      ...
      authUrl = `${webAppBaseUrl}/api/organizations/${orgUuid}/mcp/start-auth/${serverId}?product_surface=${productSurface}`
    } else {
      // Fall back to settings/connectors if we don't have the required IDs
      authUrl = `${webAppBaseUrl}/settings/connectors`
    }
