...
export function MCPListPanel({
  ...
  const webAppServers = React.useMemo(
    () =>
      servers
  ...
  const selectableItems = React.useMemo(() => {
    ...
    for (const server of webAppServers) {
      items.push({ type: 'server', server })
    }
    ...
    return items
  }, [serversByScope, webAppServers, agentServers, dynamicServers])
    ...
    <Box flexDirection="column">
      ...
      >
        <Box flexDirection="column">
          ...
          {webAppServers.length > 0 && (
            <Box flexDirection="column" marginBottom={1}>
              <Box paddingLeft={2}>
                <Text bold>claude.ai</Text>
              </Box>
              {webAppServers.map(server => renderServerItem(server))}
            </Box>
          )}
