...
export function MCPSettings({ onComplete }: Props): React.ReactNode {
  ...
  useEffect(() => {
    ...
    if (servers.length === 0 && agentMcpServers.length === 0) {
      onComplete(
        'No MCP servers configured. Please run /doctor if this is unexpected. Otherwise, run `aura mcp --help` or visit https://code.claude.com/docs/en/mcp to learn more.',
      )
    }
  }, [
    servers.length,
