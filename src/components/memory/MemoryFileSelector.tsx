...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/memoryFiles.js'
...
export function MemoryFileSelector({
  ...
  // Create entries for User and Project ANGSHENG.md even if they don't exist
  const userMemoryPath = join(getAppConfigHomeDir(), 'ANGSHENG.md')
  const projectMemoryPath = join(getOriginalCwd(), 'ANGSHENG.md')
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === 'User' && !file.isNested) {
      description = 'Saved in ~/.angsheng/ANGSHENG.md'
    } else if (
      file.type === 'Project' &&
      !file.isNested &&
      file.path === projectMemoryPath
    ) {
      description = `${isGit ? 'Checked in at' : 'Saved in'} ./ANGSHENG.md`
    } else if (file.parent) {
