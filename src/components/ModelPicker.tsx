...
export type Props = {
   ...
   * project-scoped (written to the assistant's .angsheng/settings.json via
   * install.ts) and should not leak to the user's global ~/.angsheng/settings.
   */
...
export function ModelPicker({
  ...
  const selectOptions = useMemo(
    () => {
      ...
      // Show custom model input for OpenAI provider and third-party Anthropic proxies
      ...
      if (showCustomInput) {
        mapped.push({
          ...
          onChange: (val: string) => setCustomModelInput(val),
  ...
  function handleSelect(value: string): void {
    ...
    if (!skipSettingsWrite) {
      ...
      // global ~/.angsheng/settings.json), and NOT AppState.effortValue (which
    ...
    <Box flexDirection="column">
      <Box flexDirection="column">
        <Box marginBottom={1} flexDirection="column">
          ...
          <Text dimColor>
            {headerText ??
              'Switch between Claude models. Applies to this session and future Agent Aura sessions. For other/previous model names, specify with --model.'}
          </Text>
          {sessionModel && (
            <Text dimColor>
