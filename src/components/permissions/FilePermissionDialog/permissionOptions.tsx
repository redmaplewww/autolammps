...
/**
 * Check if a path is within the project's .angsheng/ folder.
...
export function isInClaudeFolder(filePath: string): boolean {
  ...
  const claudeFolderPath = expandPath(`${getOriginalCwd()}/.angsheng`)
...
/**
 * Check if a path is within the global ~/.angsheng/ folder.
 * This is used to determine whether to show the special ".claude folder" permission option
 * for files in the user's home directory.
 */
export function isInGlobalClaudeFolder(filePath: string): boolean {
  const absolutePath = expandPath(filePath)
  const globalClaudeFolderPath = join(homedir(), '.angsheng')
...
export function getFilePermissionOptions({
  ...
  // Check if this is a .angsheng/ folder path (project or global)
  const inClaudeFolder = isInClaudeFolder(filePath)
  const inGlobalClaudeFolder = isInGlobalClaudeFolder(filePath)

  // Option 2: For .angsheng/ folder, show special option instead of generic session option
