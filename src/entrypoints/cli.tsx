...
// Provider credentials are loaded from .angsheng/settings.json via
// managedEnv.ts (applySafeConfigEnvironmentVariables / applyConfigEnvironmentVariables).
// Do NOT hardcode API keys or base URLs here — they override settings-based config.
...
async function main(): Promise<void> {
  ...
  const projectConfigDir = resolve(process.cwd(), '.angsheng');
  process.env.ANGSHENG_CONFIG_DIR = projectConfigDir;
  process.env.CLAUDE_CONFIG_DIR = projectConfigDir;

  // Fast-path for --version/-v: zero module loading needed
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    // MACRO.VERSION is inlined at build time
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${MACRO.VERSION} (Agent Aura)`);
    return;
  }
  ...
  if (process.argv[2] === '--angsheng-in-chrome-mcp') {
    profileCheckpoint('cli_claude_in_chrome_mcp_path');
    const { runAngshengInChromeMcpServer } = await import('../utils/angshengInChrome/mcpServer.js');
    await runAngshengInChromeMcpServer();
    return;
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const { runChromeNativeHost } = await import('../utils/angshengInChrome/chromeNativeHost.js');
  ...
  if (args[0] === 'weixin') {
    ...
    const { handleWeixinCli } = await import('@agent-aura/weixin');
    const { enableConfigs } = await import('../utils/config.js');
  ...
  if (
    ...
    const { getWebAppOAuthTokens } = await import('../utils/auth.js');
    const { getBridgeAccessToken } = await import('../bridge/bridgeConfig.js');
    if (!getWebAppOAuthTokens()?.accessToken && !getBridgeAccessToken()) {
  ...
  if (
    ...
    console.error(`[deprecated] Use: aura daemon ${mapped}${args[1] ? ' ' + args[1] : ''}`);
    profileCheckpoint('cli_daemon_path');
    const { enableConfigs } = await import('../utils/config.js');
    enableConfigs();
    const { setShellIfWindows } = await import('../utils/windowsPaths.js');
  ...
  if (feature('TEMPLATES') && (args[0] === 'new' || args[0] === 'list' || args[0] === 'reply')) {
    console.error(`[deprecated] Use: aura job ${args[0]} ${args.slice(1).join(' ')}`.trim());
    profileCheckpoint('cli_templates_path');
    const { templatesMain } = await import('../cli/handlers/templateJobs.js');
