...
import { getAppConfigHomeDir } from 'src/utils/envUtils.js'
...
function DistTagsDisplay({
  ...
  if (!distTags.latest) {
    return <Text dimColor>閳?Failed to fetch versions</Text>
  }
  return (
    <>
      {distTags.stable && <Text>閳?Stable version: {distTags.stable}</Text>}
      <Text>閳?Latest version: {distTags.latest}</Text>
    </>
  )
}
...
export function Doctor({ onDone }: Props): React.ReactNode {
  ...
  // Skip version check for development/source builds
  ...
  useEffect(() => {
    ...
    void (async () => {
      const userAgentsDir = join(getAppConfigHomeDir(), 'agents')
      const projectAgentsDir = join(getOriginalCwd(), '.angsheng', 'agents')
      ...
      if (isPidBasedLockingEnabled()) {
        const locksDir = join(getXDGStateHome(), 'angsheng', 'locks')
  ...
  const handleDismiss = useCallback(() => {
    onDone('Agent Aura diagnostics dismissed', { display: 'system' })
  }, [onDone])
  ...
  return (
    <Pane>
      <Box flexDirection="column">
        ...
        <Text>
          閳?Currently running: {diagnostic.installationType} (
          {diagnostic.version})
        </Text>
        {diagnostic.packageManager && (
          <Text>閳?Package manager: {diagnostic.packageManager}</Text>
        )}
        <Text>閳?Path: {diagnostic.installationPath}</Text>
        <Text>閳?Invoked: {diagnostic.invokedBinary}</Text>
        <Text>閳?Config install method: {diagnostic.configInstallMethod}</Text>
        <Text>
          閳?Search: {diagnostic.ripgrepStatus.working ? 'OK' : 'Not working'} (
        ...
        {diagnostic.multipleInstallations.length > 1 && (
          <>
            ...
            {diagnostic.multipleInstallations.map((install, i) => (
              <Text key={i}>
                閳?{install.type} at {install.path}
              </Text>
            ))}
          </>
        )}
      ...
      <Box flexDirection="column">
        ...
        {diagnostic.hasUpdatePermissions !== null && (
          <Text>
            閳?Update permissions:{' '}
            {diagnostic.hasUpdatePermissions ? 'Yes' : 'No (requires sudo)'}
          </Text>
        )}
        <Text>閳?Auto-update channel: {autoUpdatesChannel}</Text>
      ...
      {versionLockInfo?.enabled && (
        <Box flexDirection="column">
          ...
          {versionLockInfo.staleLocksCleaned > 0 && (
            <Text dimColor>
              閳?Cleaned {versionLockInfo.staleLocksCleaned} stale lock(s)
            </Text>
          )}
          {versionLockInfo.locks.length === 0 ? (
            <Text dimColor>閳?No active version locks</Text>
          ) : (
            versionLockInfo.locks.map((lock, i) => (
              <Text key={i}>
                閳?{lock.version}: PID {lock.pid}{' '}
      ...
      {agentInfo?.failedFiles && agentInfo.failedFiles.length > 0 && (
        <Box flexDirection="column">
          ...
          <Text color="error">
            閳?Failed to parse {agentInfo.failedFiles.length} agent file(s):
          </Text>
      ...
      {pluginsErrors.length > 0 && (
        <Box flexDirection="column">
          ...
          <Text color="error">
            閳?{pluginsErrors.length} plugin error(s) detected:
          </Text>
      ...
      {contextWarnings &&
        (contextWarnings.memoryPromptWarning ||
            ...
            {contextWarnings.memoryPromptWarning && (
              <>
                <Text>
                  {'  '}
                  <Text color="warning">
                    {figures.warning} {contextWarnings.memoryPromptWarning.message}
                  </Text>
                </Text>
                <Text>{'  '}• Files:</Text>
                {contextWarnings.memoryPromptWarning.details.map((detail, i) => (
                  <Text key={i} dimColor>
                    {'    '}• {detail}
                  </Text>
                ))}
