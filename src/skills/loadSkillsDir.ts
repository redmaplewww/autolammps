...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.angsheng', dir)
    case 'userSettings':
      return join(getAppConfigHomeDir(), dir)
...
/**
 * Parse paths frontmatter from a skill, using the same format as ANGSHENG.md rules.
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const userSkillsDir = join(getAppConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.angsheng', 'skills')
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.angsheng', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.angsheng/skills from
 ...
 * of ANGSHENG.md conditional rules.
 *
