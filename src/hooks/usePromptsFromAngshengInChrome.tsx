...
import {
  ...
  isTrackedAngshengInChromeTabId,
...
const AngshengInChromePromptNotificationSchema = lazySchema(() =>
  z.object({
 ...
 */
export function usePromptsFromAngshengInChrome(
  ...
  useEffect(() => {
    ...
    if (mcpClient) {
      mcpClient.client.setNotificationHandler(
        AngshengInChromePromptNotificationSchema(),
        notification => {
          if (mcpClientRef.current !== mcpClient) {
            return
          }
          ...
          if (
            typeof tabId !== 'number' ||
            !isTrackedAngshengInChromeTabId(tabId)
          ) {
            return
          }
