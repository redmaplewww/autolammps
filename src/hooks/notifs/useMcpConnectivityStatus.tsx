...
import { hasWebAppMcpEverConnected } from '../../services/mcp/webAppMcp.js'
...
export function useMcpConnectivityStatus({
  ...
  useEffect(() => {
    ...
    const failedWebAppClients = mcpClients.filter(
      client =>
        client.type === 'failed' &&
        client.config.type === 'claudeai-proxy' &&
        hasWebAppMcpEverConnected(client.name),
    )
    const needsAuthLocalServers = mcpClients.filter(
      client =>
        client.type === 'needs-auth' && client.config.type !== 'claudeai-proxy',
    )
    const needsAuthWebAppServers = mcpClients.filter(
      client =>
    ...
    if (
      ...
      failedWebAppClients.length === 0 &&
      needsAuthLocalServers.length === 0 &&
      needsAuthWebAppServers.length === 0
    ) {
      return
    }
    ...
    if (failedWebAppClients.length > 0) {
      addNotification({
        key: 'mcp-claudeai-failed',
        jsx: (
          <>
            <Text color="error">
              {failedWebAppClients.length} claude.ai{' '}
              {failedWebAppClients.length === 1 ? 'connector' : 'connectors'}{' '}
              unavailable
            </Text>
            <Text dimColor> · /mcp</Text>
          </>
    ...
    if (needsAuthWebAppServers.length > 0) {
      addNotification({
        key: 'mcp-claudeai-needs-auth',
        jsx: (
          <>
            <Text color="warning">
              {needsAuthWebAppServers.length} claude.ai{' '}
              {needsAuthWebAppServers.length === 1
                ? 'connector needs'
                : 'connectors need'}{' '}
              auth
            </Text>
