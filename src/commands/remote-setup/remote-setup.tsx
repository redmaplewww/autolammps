...
function Web({ onDone }: { onDone: LocalJSXCommandOnDone }) {
    ...
    void checkLoginState().then(async result => {
      switch (result.status) {
        case 'not_signed_in':
          ...
          onDone('Not signed in to ANGSHENG. Run /login first.')
          return
  ...
  if (step.name === 'uploading') {
    return <LoadingState message="Connecting GitHub to ANGSHENG…" />
  }

  const token = step.token
  return (
    <Dialog
      title="Connect Angsheng on the web to GitHub?"
      onCancel={handleCancel}
      hideInputGuide
    >
      <Box flexDirection="column">
        <Text>
          ANGSHENG on the web requires connecting to your GitHub account to clone
