...
/**
 * Generates Angsheng specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and Angsheng content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Angsheng defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getAppConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Claude home: ${getAppConfigHomeDir()}`)
