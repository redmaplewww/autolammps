...
import { getAdditionalDirectoriesForMemoryPrompt } from '../../bootstrap/state.js'
import {
...
async function getWatchablePaths(): Promise<string[]> {
  ...
  // User skills directory (~/.angsheng/skills)
  ...
  // User commands directory (~/.angsheng/commands)
  ...
  // Project skills directory (.angsheng/skills)
  ...
  // Project commands directory (.angsheng/commands)
  ...
  for (const dir of getAdditionalDirectoriesForMemoryPrompt()) {
    const additionalSkillsPath = platformPath.join(dir, '.angsheng', 'skills')
    try {
