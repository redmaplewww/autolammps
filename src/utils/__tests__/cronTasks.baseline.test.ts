...
describe('cronTasks baseline', () => {
  ...
  test('durable cron tasks are written to .angsheng/scheduled_tasks.json', async () => {
    ...
    expect(filePath).toBe(join(tempDir, '.angsheng', 'scheduled_tasks.json'))
  ...
  test('writeCronTasks strips runtime-only durable/agentId flags but persists creatorSessionId', async () => {
    await writeCronTasks([
      {
        ...
        durable: false,
        agentId: 'teammate-1',
        creatorSessionId: 'session-abc-123',
      },
    ])
    ...
    expect(raw).toContain('"creatorSessionId"')
    expect(raw).toContain('"session-abc-123"')

    const tasks = await readCronTasks()
    expect(tasks[0]?.creatorSessionId).toBe('session-abc-123')
  })
  ...
  test('durable cron tasks persist creatorSessionId through read/write cycle', async () => {
    const id = await addCronTask(
      '* * * * *',
      'session-tagged prompt',
      true,
      true,
      undefined,
      'my-session-test',
    ...
    const fileTasks = await readCronTasks()
    expect(fileTasks).toHaveLength(1)
    expect(fileTasks[0].id).toBe(id)
    expect(fileTasks[0].creatorSessionId).toBe('my-session-test')

    const raw = await readFile(getCronFilePath(), 'utf-8')
    expect(raw).toContain('"creatorSessionId"')
    expect(raw).toContain('"my-session-test"')
  })
