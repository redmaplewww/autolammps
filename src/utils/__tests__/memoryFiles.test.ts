...
describe("isMemoryFilePath", () => {
  test("returns true for ANGSHENG.md path", () => {
    expect(isMemoryFilePath("/project/ANGSHENG.md")).toBe(true);
  });

  test("returns true for ANGSHENG.local.md path", () => {
    expect(isMemoryFilePath("/project/ANGSHENG.local.md")).toBe(true);
  });

  test("returns true for .angsheng/rules/ path", () => {
    expect(isMemoryFilePath("/project/.angsheng/rules/foo.md")).toBe(true);
  });
  ...
  test("returns false for .claude directory non-rules file", () => {
    expect(isMemoryFilePath("/project/.angsheng/settings.json")).toBe(false);
  });
  ...
  test("returns false for non-.md file in .angsheng/rules/", () => {
    expect(isMemoryFilePath(".angsheng/rules/foo.txt")).toBe(false);
  });
});
