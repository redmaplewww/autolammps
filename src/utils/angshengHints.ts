/**
 * Angsheng hints protocol.
 *
 * CLIs and SDKs running under Angsheng can emit a self-closing
...
export type AppHintType = 'plugin'
...
export function extractClaudeCodeHints(
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[appHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[appHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[appHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as AppHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: AppHint | null = null
...
export function setPendingHint(hint: AppHint): void {
...
export function getPendingHintSnapshot(): AppHint | null {
  return pendingHint
}
