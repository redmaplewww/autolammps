...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.angsheng/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.angsheng/settings.json)'
    case 'projectSettings':
      return 'Project settings (.angsheng/settings.json)'
    case 'localSettings':
      return 'Local settings (.angsheng/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.angsheng/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.angsheng/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by Angsheng)'
    default:
