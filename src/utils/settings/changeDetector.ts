 ...
 * it's assumed to be from Angsheng itself and won't trigger a notification.
 */
...
function getSourceForPath(path: string): SettingSource | undefined {
  ...
  return getEnabledSettingSources().find(
...
function startMdmPoll(): void {
  if (!isSettingSourceEnabled('policySettings')) return
