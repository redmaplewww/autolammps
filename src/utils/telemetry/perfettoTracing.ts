/**
 * Perfetto Tracing for Angsheng (Ant-only)
 *
 ...
 * 3. Run Angsheng normally
 * 4. Trace file is written to ~/.angsheng/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    if (isEnvTruthy(envValue)) {
      const tracesDir = join(getAppConfigHomeDir(), 'traces')
