// Scheduler lease lock for .angsheng/scheduled_tasks.json.
//
// When multiple Angsheng sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.angsheng', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .angsheng/ doesn't exist yet — create it and retry once. In steady
