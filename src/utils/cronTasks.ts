// Scheduled prompts, stored in <project>/.angsheng/scheduled_tasks.json.
//
...
//
// File format:
//   { "tasks": [{ id, cron, prompt, createdAt, recurring?, permanent?, creatorSessionId? }] }
...
export type CronTask = {
  ...
  agentId?: string
  /**
   * Session ID of the session that created this task. Persisted to disk for
   * durable tasks so the scheduler can route fires to the correct session
   * when multiple CLI sessions share a project. The scheduler only fires
   * file-backed tasks whose creatorSessionId matches the current session,
   * preventing cross-session interference. Absent on legacy tasks (created
   * before this field existed) — those always fire regardless of session,
...
const CRON_FILE_REL = join('.angsheng', 'scheduled_tasks.json')
...
/**
 * Read and parse .angsheng/scheduled_tasks.json. Returns an empty task list if the file
...
export async function readCronTasks(dir?: string): Promise<CronTask[]> {
  ...
  for (const t of file.tasks) {
    ...
    out.push({
      ...
      ...(typeof t.creatorSessionId === 'string'
...
/**
 * Overwrite .angsheng/scheduled_tasks.json with the given tasks. Creates .angsheng/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.angsheng'), { recursive: true })
  // Strip runtime-only flags (`durable`, `agentId`) — everything on disk is
  // durable by definition, and keeping them out means readCronTasks() naturally
  // yields durable: undefined / agentId: undefined without having to clear.
  // `creatorSessionId` IS persisted — it is the on-disk routing key for
  // cross-session isolation and must survive restarts.
  const body: CronFile = {
    tasks: tasks.map(
 ...
 * written to .angsheng/scheduled_tasks.json and dies with the process. The
...
export async function addCronTask(
  ...
  // Tag durable tasks with the creating session so the scheduler can route
  // fires to the correct session when multiple sessions share a project.
