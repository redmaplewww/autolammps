 ...
 * to the user's ~/.angsheng/settings.json API-key config (apiKeyHelper,
...
export function getAuthTokenSource() {
  ...
  const oauthTokens = getWebAppOAuthTokens()
...
export function getAnthropicApiKeyWithSource(
  ...
  // Third-party proxy: skip approval check — user explicitly configured the key
  if (!isFirstPartyAnthropicBaseUrl() && apiKeyEnv) {
    return {
      key: apiKeyEnv,
      source: 'ANTHROPIC_API_KEY',
    }
 ...
 * from ~/.angsheng/settings.json or project settings is ignored.
 */
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  try {
    ...
    getWebAppOAuthTokens.cache?.clear?.()
    clearBetasCaches()
...
export const getWebAppOAuthTokens = memoize((): OAuthTokens | null => {
...
async function invalidateOAuthCacheIfDiskChanged(): Promise<void> {
  try {
    const { mtimeMs } = await stat(
      join(getAppConfigHomeDir(), '.credentials.json'),
    )
...
async function handleOAuth401ErrorImpl(
  ...
  clearOAuthTokenCache()
  const currentTokens = await getWebAppOAuthTokensAsync()
 ...
 */
export async function getWebAppOAuthTokensAsync(): Promise<OAuthTokens | null> {
  if (isBareMode()) return null
...
async function checkAndRefreshOAuthTokenIfNeededImpl(
  ...
  getWebAppOAuthTokens.cache?.clear?.()
  clearKeychainCache()
  const freshTokens = await getWebAppOAuthTokensAsync()
  if (
  ...
  const claudeDir = getAppConfigHomeDir()
  ...
  try {
    ...
    getWebAppOAuthTokens.cache?.clear?.()
    clearKeychainCache()
    const lockedTokens = await getWebAppOAuthTokensAsync()
    if (
...
export function isWebAppSubscriber(): boolean {
  if (!isAnthropicAuthEnabled()) {
    return false
  }

  return shouldUseClaudeAIAuth(getWebAppOAuthTokens()?.scopes)
}
...
export function hasProfileScope(): boolean {
  return (
    getWebAppOAuthTokens()?.scopes?.includes(CLAUDE_AI_PROFILE_SCOPE) ?? false
  )
}
...
export function isOverageProvisioningAllowed(): boolean {
  ...
  if (!isWebAppSubscriber() || !billingType) {
    return false
  }
