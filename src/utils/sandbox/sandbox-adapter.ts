...
/**
 * Resolve Angsheng-specific path patterns for sandbox-runtime.
 *
 * Angsheng uses special path prefixes in permission rules:
...
/**
 * Convert Angsheng settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // This blocks settings in the original working directory (where Angsheng started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.angsheng', 'settings.json'))
    denyWrite.push(resolve(cwd, '.angsheng', 'settings.local.json'))
  }

  // Block writes to .angsheng/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .angsheng/commands and
  // .angsheng/agents but not .angsheng/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Claude capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.angsheng', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.angsheng', 'skills'))
  }
