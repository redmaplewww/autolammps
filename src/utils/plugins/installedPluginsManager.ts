 ...
 *
 * The enabled/disabled state remains in .angsheng/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.angsheng/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.angsheng/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.angsheng/plugins/cache/{marketplace}/{plugin}/{version}
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = findExistingPluginManifestPath(
    pluginCachePath,
    'plugin.json',
  )
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if a plugin manifest directory is present
          if (
            dirEntries.includes(PRIMARY_PLUGIN_MANIFEST_DIR) ||
            dirEntries.includes('.claude-plugin')
          ) {
