...
} from '@agent-aura/builtin-tools/tools/AgentTool/agentMemory.js'
...
async function loadAgentFromFile(
  ...
  try {
    ...
    // time. For this level of control, define the agent in .angsheng/agents/
    ...
    for (const field of ['permissionMode', 'hooks', 'mcpServers'] as const) {
      if (frontmatter[field] !== undefined) {
        logForDebugging(
          `Plugin agent file ${filePath} sets ${field}, which is ignored for plugin agents. Use .angsheng/agents/ for this level of control.`,
          { level: 'warn' },
        )
      }
    }
