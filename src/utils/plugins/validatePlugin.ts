...
import {
  findExistingPluginManifestPath,
  getDefaultMarketplaceManifestRelativePath,
  getDefaultPluginManifestRelativePath,
...
function detectManifestType(
  ...
  // Check if it's in a plugin manifest directory
...
function marketplaceSourceHint(p: string): string {
  ...
  return (
    ...
    `containing ${getDefaultMarketplaceManifestRelativePath().replace('/marketplace.json', '/')}, not relative to marketplace.json. ` +
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Angsheng ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Angsheng accepts ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        isPluginManifestDirName(path.basename(manifestDir))
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        const pluginJsonPath = findExistingPluginManifestPath(
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/${getDefaultPluginManifestRelativePath()} says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in the plugin manifest directory
    // Prefer marketplace.json over plugin.json
    const marketplacePath = findExistingPluginManifestPath(
      absolutePath,
      'marketplace.json',
    )
    ...
    const pluginPath = findExistingPluginManifestPath(
      absolutePath,
      'plugin.json',
    )
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected ${getDefaultMarketplaceManifestRelativePath()} or ${getDefaultPluginManifestRelativePath()}`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
