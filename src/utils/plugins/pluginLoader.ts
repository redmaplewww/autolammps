 ...
 *
 * This module is responsible for discovering, loading, and validating Angsheng plugins
...
import { findExistingPluginManifestPath } from './manifestPaths.js'
import {
 ...
 * Format: ~/.angsheng/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.angsheng/plugins/cache/{plugin-name}/
 *
...
export async function cachePlugin(
  ...
  const manifestPath = findExistingPluginManifestPath(tempPath, 'plugin.json')
...
export async function createPluginFromPath(
  ...
  const manifestPath = findExistingPluginManifestPath(pluginPath, 'plugin.json')
