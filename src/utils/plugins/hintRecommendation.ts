 ...
 *
 * State persists in GlobalConfig.appHints — a show-once record per
...
/**
 * Hard cap on `appHints.plugin[]` — bounds config growth. Each shown
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.appHints?.plugin ?? []
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.appHints?.disabled) return current
    return {
      ...current,
      appHints: { ...current.appHints, disabled: true },
    }
  })
}
