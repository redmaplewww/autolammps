/**
 * Marketplace manager for Angsheng plugins
 *
 ...
 *
 * Reads the configuration file at ~/.angsheng/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.angsheng/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.angsheng/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Angsheng bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.angsheng/plugins/known_marketplaces.json,
  ...
  try {
    switch (source.source) {
      ...
      case 'directory': {
        ...
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = findExistingPluginManifestPath(
          absPath,
          'marketplace.json',
        )
      ...
      case 'settings': {
        ...
        marketplacePath = getPrimaryPluginManifestPath(
          temporaryCachePath,
          'marketplace.json',
        )
        cleanupNeeded = false
 ...
 * The configuration is saved to ~/.angsheng/plugins/known_marketplaces.json.
 *
...
async function readCachedMarketplace(
  ...
  const nestedPath = findExistingPluginManifestPath(
    installLocation,
    'marketplace.json',
  )
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Angsheng version. Run 'claude marketplace remove ${name}' and ` +
