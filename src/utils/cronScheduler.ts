// Non-React scheduler core for .angsheng/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .angsheng/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    const missed = findMissedTasks(next, now).filter(
      t =>
        ...
        (!filter || filter(t)) &&
        // Don't surface missed tasks from another session (REPL path only).
        // Daemon callers use their own filter option.
        (dir !== undefined ||
          t.creatorSessionId === undefined ||
          t.creatorSessionId === getSessionId()),
    )
    if (missed.length > 0) {
  ...
  function check() {
    ...
    function process(t: CronTask, isSession: boolean) {
      ...
      // File-backed tasks: skip if tagged with a creatorSessionId that doesn't
      // match our session. This prevents cross-session interference when
      // multiple CLI sessions share a project — each session only fires its own
      // durable tasks. Legacy tasks (no creatorSessionId) always fire.
      if (
        !isSession &&
        t.creatorSessionId !== undefined &&
        t.creatorSessionId !== getSessionId()
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    ...
    `${plural ? 'They have' : 'It has'} already been removed from .angsheng/scheduled_tasks.json.\n\n` +
