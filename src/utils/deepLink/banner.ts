 ...
 * notice which directory — and therefore which ANGSHENG.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which ANGSHENG.md
 ...
 * and whether its ANGSHENG.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — ANGSHENG.md may be stale' : ''}`,
    )
  }
