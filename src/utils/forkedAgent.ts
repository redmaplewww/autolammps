...
/**
 * Maximum tokens for sub-agent result text returned to the parent coordinator.
 * Keeping this small prevents coordinator context overflow while preserving
 * the structured RESULT block that sub-agents are required to output.
 */
const MAX_SUBAGENT_RESULT_TOKENS = 3000

/**
 * Extracts result text from agent messages.
 * Truncates to MAX_SUBAGENT_RESULT_TOKENS to prevent coordinator context overflow.
 */
export function extractResultText(
  ...
  if (!textContent) return defaultText

  const tokens = roughTokenCountEstimation(textContent)
  if (tokens <= MAX_SUBAGENT_RESULT_TOKENS) {
    return textContent
  }
  ...
  for (const line of lines) {
    const lineTokens = roughTokenCountEstimation(line)
    if (keptTokens + lineTokens > MAX_SUBAGENT_RESULT_TOKENS * 0.9) break
    kept.push(line)
  ...
  return (
    kept.join('\n') +
    `\n\n[RESULT TRUNCATED: ${tokens} tokens → ${MAX_SUBAGENT_RESULT_TOKENS} token limit. Full output written to scratchpad/ by this agent.]`
  )
}
