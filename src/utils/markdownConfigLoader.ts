 ...
 * stopping there makes the parent project's `.angsheng/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.angsheng/worktrees/`) stay on the old behavior: their `.git`
...
/**
 * Traverses from the current directory up to the project boundary,
 * collecting .angsheng directories inside this project only.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.angsheng/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .angsheng/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  subdir: ClaudeConfigDirectory,
  cwd: string,
): string[] {
  const stopBoundary = resolveStopBoundary(cwd) ?? resolve(cwd)
  let current = resolve(cwd)
  const dirs: string[] = []

  // Traverse from current directory up to the current project's boundary.
  while (true) {
    const claudeSubdir = join(current, '.angsheng', subdir)
    ...
    // Stop after processing the project boundary. This prevents commands from
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getAppConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.angsheng', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .angsheng/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .angsheng/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .angsheng/<subdir> content — loading the main
    ...
    const [managedFiles, userFiles, projectFilesNested] = await Promise.all([
      // Load managed files only when policy settings are enabled.
      isSettingSourceEnabled('policySettings')
    ...
    // In this fork managed/user discovery is disabled by setting-source gates.
    const allFiles = [...managedFiles, ...userFiles, ...projectFiles]

    // Deduplicate files that resolve to the same physical file (same inode).
    // This prevents the same file from appearing multiple times when symlinks
    // point to a directory within the project hierarchy.
 ...
 * @param dir Directory (eg. "~/.angsheng/commands")
 * @returns Array of parsed markdown files with metadata
 */
