...
import { renderToolResultMessage as renderDefaultMCPToolResultMessage } from '@agent-aura/builtin-tools/tools/MCPTool/UI.js'
import type { MCPToolResult } from '../../utils/mcpValidation.js'
import { truncateToWidth } from '../format.js'
import { trackAngshengInChromeTabId } from './common.js'

export type { Tool } from '@modelcontextprotocol/sdk/types.js'

/**
 * All tool names from BROWSER_TOOLS in @ant/chrome-mcp-bridge.
...
function renderChromeToolUseMessage(
  ...
  if (typeof tabId === 'number') {
    trackAngshengInChromeTabId(tabId)
  }
...
/**
 * Renders a clickable "View Tab" link for Angsheng in Chrome MCP tools.
 * Returns null if:
 * - The tool is not a Angsheng in Chrome MCP tool
...
/**
 * Returns tool method overrides for Angsheng in Chrome MCP tools. Use this to customize
 * rendering for chrome tools in a single spread operation.
 */
export function getAngshengInChromeMCPToolOverrides(toolName: string): {
  ...
  return {
    userFacingName(_input?: Record<string, unknown>) {
      ...
      return `Angsheng in Chrome[${displayName}]`
    },
    renderToolUseMessage(
      input: Record<string, unknown>,
