      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Angsheng in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isAngshengInChromeMCPServer(name: string): boolean {
...
export function trackAngshengInChromeTabId(tabId: number): void {
...
export function isTrackedAngshengInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Angsheng in Chrome] No compatible browser found')
    return false
  }
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- ClaudeForChromeContext.getSocketPaths (external @ant/chrome-mcp-bridge) requires a sync () => string[] callback
...
function getSocketName(): string {
  // NOTE: This must match the one used in the Angsheng in Chrome MCP
