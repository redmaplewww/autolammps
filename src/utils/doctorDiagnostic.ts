...
export async function getCurrentInstallationType(): Promise<InstallationType> {
  ...
  // Running from source (e.g. bun src/entrypoints/cli.tsx)
  if (invokedPath.includes('src/entrypoints/')) {
    return 'development'
  }
...
> {
  ...
  const localPath = join(homedir(), '.angsheng', 'local')
    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias claude="~/.angsheng/local/claude"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias claude="~/.angsheng/local/claude"',
        })
      }
    }
  }
