...
import { isAgentMemoryPath } from '@agent-aura/builtin-tools/tools/AgentTool/agentMemory.js'
import {
  CLAUDE_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_CLAUDE_FOLDER_PERMISSION_PATTERN,
} from '@agent-aura/builtin-tools/tools/FileEditTool/constants.js'
...
/**
 * If filePath is inside a .angsheng/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .angsheng/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.angsheng', 'skills')),
      prefix: '/.angsheng/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.angsheng', 'skills')),
      prefix: '~/.angsheng/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.angsheng/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.angsheng/./settings.json` which would evade the endsWith() check
  ...
  if (
    ...
    // Include .angsheng/settings.json even for other projects
    return true
  }
...
// Always ask when Angsheng tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  if (isClaudeSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .angsheng/commands or .angsheng/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.angsheng', 'commands')
  const agentsDir = join(getOriginalCwd(), '.angsheng', 'agents')
  const skillsDir = join(getOriginalCwd(), '.angsheng', 'skills')
 ...
 * Path format: ~/.angsheng/projects/{sanitized-cwd}/...
 */
 ...
 * This is a per-user temporary directory used by Angsheng for all temp files.
 *
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .angsheng/worktrees/ is a structural path (where Claude stores
 ...
 * - Claude config files (.angsheng/settings.json, .angsheng/commands/, .angsheng/agents/)
 * - MCP CLI state files (managed internally by Angsheng)
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .angsheng/)
    return {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // 1.6. Check for .angsheng/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .angsheng/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .angsheng/ folder.
  //
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .angsheng/ (project or global).
    // Accepts both the broad patterns ('/.angsheng/**', '~/.angsheng/**') and
    // narrowed ones like '/.angsheng/skills/my-skill/**' so users can grant
    ...
    // '/.angsheng/../**' from leaking this bypass outside .angsheng/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .angsheng/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.angsheng/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.angsheng/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      const jobsRoot = join(getAppConfigHomeDir(), 'jobs')
  ...
  // ~/.angsheng/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .angsheng/launch.json — desktop preview config (dev server command + port).
  ...
  // .angsheng/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .angsheng/ only (not ~/.angsheng/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.angsheng', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.angsheng/tasks/) for swarm task coordination
  const tasksDir = join(getAppConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.angsheng/teams/) for swarm coordination
  const teamsReadDir = join(getAppConfigHomeDir(), 'teams') + sep
  if (
