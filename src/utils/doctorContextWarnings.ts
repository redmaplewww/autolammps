...
import type { AgentDefinitionsResult } from '@agent-aura/builtin-tools/tools/AgentTool/loadAgentsDir.js'
...
async function checkMemoryPromptFiles(): Promise<ContextWarning | null> {
  ...
  const message =
    largeFiles.length === 1
      ? `Large ANGSHENG.md file detected (${largeFiles[0]!.content.length.toLocaleString()} chars > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()})`
      : `${largeFiles.length} large ANGSHENG.md files detected (each > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()} chars)`
...
export async function checkContextWarnings(
  ...
  const [memoryPromptWarning, agentWarning, mcpWarning, unreachableRulesWarning] =
    await Promise.all([
      checkMemoryPromptFiles(),
