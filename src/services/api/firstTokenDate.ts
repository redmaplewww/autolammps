...
/**
 * Fetch the user's first Angsheng token date and store in config.
 * This is called after successful login to cache when they started using Angsheng.
 */
export async function fetchAndStoreAppFirstTokenDate(): Promise<void> {
  try {
    const config = getGlobalConfig()

    if (config.appFirstTokenDate !== undefined) {
      return
    }
    ...
    saveGlobalConfig(current => ({
      ...current,
      appFirstTokenDate: firstTokenDate,
    }))
  } catch (error) {
    logError(error)
  }
