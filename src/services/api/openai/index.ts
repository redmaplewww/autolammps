...
import type { Options } from '../anthropic.js'
...
function formatOpenAIProviderError(
  error: unknown,
  ...
  const diagnostics = [
    ...
    `- Resolved model: ${resolvedModel || '(unknown)'}`,
  ]
  ...
  if (baseURL) {
    try {
      ...
      if (parsed.pathname === '' || parsed.pathname === '/') {
        diagnostics.push(
          `- Hint: this endpoint usually needs a /v1 suffix. Try: ${parsed.origin}/v1`,
        )
      }
    } catch {
      diagnostics.push(
        '- Hint: OPENAI_BASE_URL is not a valid URL. Use the full OpenAI-compatible endpoint, usually ending in /v1.',
      )
    }
  } else {
    diagnostics.push(
      '- Hint: set OPENAI_BASE_URL to the full OpenAI-compatible endpoint, usually ending in /v1.',
    )
  }

  if (!process.env.OPENAI_MODEL) {
    diagnostics.push(
      '- Hint: set OPENAI_MODEL or run /model with a model ID your gateway actually exposes.',
    )
  }

  return [`API Error: ${message}`, '', ...diagnostics].join('\n')
}
...
> {
  ...
  try {
    ...
    yield createAssistantAPIErrorMessage({
      content: formatOpenAIProviderError(error, openaiModel),
      apiError: 'api_error',
