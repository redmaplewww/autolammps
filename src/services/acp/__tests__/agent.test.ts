...
const actualTool = await import('../../../Tool.ts')
const actualFileStateCache = await import('../../../utils/fileStateCache.ts')
const actualAcpUtils = await import('../utils.ts')
const actualModel = await import('../../../utils/model/model.ts')
const actualBridge = await import('../bridge.ts')
...
mock.module('../../../utils/fileStateCache.js', () => {
  return {
    ...actualFileStateCache,
    createFileStateCacheWithSizeLimit: mock(() => ({
      max: 1,
      maxSize: 1,
      dump: () => [],
      load: () => {},
    })),
    cloneFileStateCache: mock((cache: unknown) => cache),
  }
})
...
const mockGetSmallFastModel = mock(() => 'claude-haiku-4-5')

mock.module('../../../utils/model/model.js', () => {
  return {
    ...actualModel,
    getMainLoopModel: mockGetMainLoopModel,
    getSmallFastModel: mockGetSmallFastModel,
  }
})
