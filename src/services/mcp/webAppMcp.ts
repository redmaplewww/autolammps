...
import { getWebAppOAuthTokens } from 'src/utils/auth.js'
...
export const fetchClaudeAIMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // Check for user:mcp_servers scope directly instead of isWebAppSubscriber().
      // In non-interactive mode, isWebAppSubscriber() returns false when ANTHROPIC_API_KEY
 ...
 */
export function markWebAppMcpConnected(name: string): void {
  saveGlobalConfig(current => {
    const seen = current.webAppMcpEverConnected ?? []
    if (seen.includes(name)) return current
    return { ...current, webAppMcpEverConnected: [...seen, name] }
  })
}

export function hasWebAppMcpEverConnected(name: string): boolean {
  return (getGlobalConfig().webAppMcpEverConnected ?? []).includes(name)
}
