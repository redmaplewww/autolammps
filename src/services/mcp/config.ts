...
import { fetchClaudeAIMcpConfigsIfEligible } from './webAppMcp.js'
 ...
 */
export function dedupWebAppMcpServers(
  webAppServers: Record<string, ScopedMcpServerConfig>,
  manualServers: Record<string, ScopedMcpServerConfig>,
): {
  servers: Record<string, ScopedMcpServerConfig>
  suppressed: Array<{ name: string; duplicateOf: string }>
  ...
  const servers: Record<string, ScopedMcpServerConfig> = {}
  const suppressed: Array<{ name: string; duplicateOf: string }> = []
  for (const [name, config] of Object.entries(webAppServers)) {
    const sig = getMcpServerSignature(config)
...
export async function addMcpConfig(
  ...
  if (isAngshengInChromeMCPServer(name)) {
...
/**
 * Get Angsheng MCP configurations (excludes claude.ai servers from the
 ...
 * @returns Angsheng server configurations with appropriate scopes
 */
...
export async function getAllMcpConfigs(): Promise<{
  ...
  const { servers: dedupedWebApp } = dedupWebAppMcpServers(
    claudeaiMcpServers as Record<string, ScopedMcpServerConfig>,
    claudeCodeServers,
  )

  // Merge with claude.ai having lowest precedence
  const servers = Object.assign({}, dedupedWebApp, claudeCodeServers)
