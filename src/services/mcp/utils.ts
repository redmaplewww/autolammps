 ...
 * capabilities display â€?skills are a separate feature shown in `/skills`,
 ...
 * Excludes `scope` (provenance, not content â€?moving a server from .mcp.json
 ...
 *   - config hash changed (args/url/env edited in .mcp.json) â€?any scope
 *
 ...
 * config-changed case applies to all scopes â€?if the config actually changed
...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .angsheng/settings.json).
