...
import { type MCPProgress, MCPTool } from '@agent-aura/builtin-tools/tools/MCPTool/MCPTool.js'
import { createMcpAuthTool } from '@agent-aura/builtin-tools/tools/McpAuthTool/McpAuthTool.js'
...
} from '@agent-aura/mcp-client'
import { recursivelySanitizeUnicode } from '@agent-aura/mcp-client'
...
import { classifyMcpToolForCollapse } from '@agent-aura/builtin-tools/tools/MCPTool/classifyForCollapse.js'
...
import { markWebAppMcpConnected } from './webAppMcp.js'
...
import { isAngshengInChromeMCPServer } from '../../utils/angshengInChrome/common.js'

// Lazy: toolRendering.tsx pulls React/ink; only needed when Claude-in-Chrome MCP server is connected
/* eslint-disable @typescript-eslint/no-require-imports */
const angshengInChromeToolRendering =
  (): typeof import('../../utils/angshengInChrome/toolRendering.js') =>
    require('../../utils/angshengInChrome/toolRendering.js')
...
function getMcpAuthCachePath(): string {
  return join(getAppConfigHomeDir(), 'mcp-needs-auth-cache.json')
}
 ...
 */
export function createWebAppProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      await checkAndRefreshOAuthTokenIfNeeded()
      const currentTokens = getWebAppOAuthTokens()
      if (!currentTokens) {
      ...
      // Return the exact token that was sent. Reading getWebAppOAuthTokens()
    ...
    if (!tokenChanged) {
      ...
      const now = getWebAppOAuthTokens()?.accessToken
  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        // eslint-disable-next-line eslint-plugin-n/no-unsupported-features/node-builtins
        const fetchWithAuth = createWebAppProxyFetch(globalThis.fetch)
        ...
        isAngshengInChromeMCPServer(name)
      ) {
        // Run the Chrome MCP server in-process to avoid spawning a ~325 MB subprocess
        const { createChromeContext } = await import(
          '../../utils/angshengInChrome/mcpServer.js'
        )
        const { createClaudeForChromeMcpServer } = await import(
          '@ant/chrome-mcp-bridge'
        )
      ...
      return toolsToProcess
        .map((tool): Tool => {
          ...
          return {
            ...
            ...(isAngshengInChromeMCPServer(client.name) &&
            (client.config.type === 'stdio' || !client.config.type)
              ? angshengInChromeToolRendering().getAngshengInChromeMCPToolOverrides(
                  tool.name,
                )
              : {}),
            ...(feature('CHICAGO_MCP') &&
