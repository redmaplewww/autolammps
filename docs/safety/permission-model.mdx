...
description: "详解 Angsheng 的三级权限模型实现：基于 src/utils/permissions/permissions.ts 的规则匹配引擎、五层规则来源优先级、工具名/命令/路径三维度匹配、Denial Tracking 死循环防护、权限模式切换机制。"
...
```
1. userSettings   — ~/.angsheng/settings.json（跨项目）
2. projectSettings — .angsheng/settings.json（团队共享）
3. localSettings  — .angsheng/settings.local.json（gitignored，个人覆盖）
