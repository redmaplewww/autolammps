...
description: "揭秘 Angsheng 任务管理系统的双轨架构：V1 内存 TodoWrite 与 V2 文件系统 Tasks，包含依赖管理、认领竞争和验证推动机制。"
...
Angsheng 的任务管理并非单一系统，而是两个并存、按运行模式切换的实现：
...
| **存储** | 内存中 `AppState.todos[sessionId]`（Zustand store） | 文件系统 `~/.angsheng/tasks/<taskListId>/<id>.json` |
...
每个任务是一个独立 JSON 文件，路径为 `~/.angsheng/tasks/<taskListId>/<id>.json`：
