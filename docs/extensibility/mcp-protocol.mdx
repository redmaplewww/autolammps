...
description: "从源码角度解析 Angsheng 的 MCP 集成：内置 MCP 与外部 MCP 的区别、7 种传输层实现、connectToServer 的 memoize 缓存、工具发现的 LRU 策略、认证状态机、以及 MCP 工具如何进入权限检查链路。"
...
配置层（多来源合并）
  ...
  ├── setupComputerUseMCP() / setupAngshengInChrome()                                   ← 内置（动态注册）
...
Angsheng 的 MCP 实现区分 **内置 MCP 服务器** 和 **外部 MCP 服务器**。两者使用相同的客户端协议和工具发现机制，但在连接方式、生命周期管理和配置来源上完全不同。

### 内置 MCP 服务器

内置 MCP 服务器由 Angsheng 自身提供，无需用户手动配置。它们在启动时自动注册为 `dynamic` scope 的配置，并在同进程内运行。

| 服务器 | 名称 | 包路径 | Feature Flag | 启用方式 |
|--------|------|--------|-------------|---------|
| Computer Use | `computer-use` | `@ant/computer-use-mcp` | `CHICAGO_MCP` | GrowthBook gate + macOS + interactive |
| Angsheng in Chrome | `claude-in-chrome` | `@ant/chrome-mcp-bridge` | — | `--chrome` 参数或 `angshengInChromeDefaultEnabled` 配置 |
...
if (isAngshengInChromeMCPServer(name)) {
  const { createChromeContext } = await import('../../utils/angshengInChrome/mcpServer.js')
  const { createClaudeForChromeMcpServer } = await import('@ant/chrome-mcp-bridge')
...
VSCode SDK MCP 是特殊的内置模式。IDE（如 VS Code、JetBrains）通过嵌入方式启动 Angsheng，并传入 `type:'sdk'` 的 MCP 配置。这类配置：
...
| 本地配置 | `local` | `<project>/.angsheng/settings.local.json` | 高 |
| 用户配置 | `user` | `~/.angsheng/settings.json` | 中 |
...
`fetchToolsForClient()`（`client.ts:1744-2000`）使用 `memoizeWithLRU` 缓存（上限 100），将 MCP 工具转换为 Angsheng 的统一 Tool 接口：
...
- **Angsheng in Chrome**：`createClaudeForChromeMcpServer()` 在 `@ant/chrome-mcp-bridge` 包中构建 Server，提供 17+ 个浏览器控制工具。
...
| **配置来源** | `setupComputerUseMCP()` / `setupAngshengInChrome()` 等动态注册 | settings.json / .mcp.json / 插件 / claude.ai |
...
| `src/utils/angshengInChrome/mcpServer.ts` | Chrome MCP Server 构建 + Bridge 配置 |
| `src/tools/MCPTool/MCPTool.ts` | MCP 工具包装：统一 Tool 接口 |
| `src/entrypoints/mcp.ts` | MCP server 入口（Angsheng 作为 MCP server） |
