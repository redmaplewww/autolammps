...
description: "详细说明 Angsheng MCP 配置的来源层次、合并优先级、传输类型、企业策略管控、插件集成和保留名称机制。"
keywords: ["MCP", "配置", "settings.json", ".mcp.json", "企业策略", "插件"]
---

## 配置来源与作用域

Angsheng 的 MCP 配置来自多个来源，每个来源对应一个 `scope`（作用域）。配置按优先级合并，高优先级来源的同名配置覆盖低优先级。
...
| 本地项目 | `local` | `<project>/.angsheng/settings.local.json` | 项目级私有配置（不提交到 VCS） |
| 项目配置 | `project` | `<project>/.mcp.json` | 项目级共享配置（可提交到 VCS） |
| 用户全局 | `user` | `~/.angsheng/settings.json` | 用户级配置，所有项目共享 |
...
首次打开项目时，Angsheng 会提示用户审批 `.mcp.json` 中的每个服务器。审批状态持久化在本地配置中。
...
claude.ai 连接器使用相同的内容签名机制去重（`dedupWebAppMcpServers`）：
...
### Angsheng in Chrome MCP

```typescript
// 类似 Computer Use，在 main.tsx 中注册
const { mcpConfig, allowedTools, systemPrompt } = setupAngshengInChrome()
dynamicMcpConfig = { ...dynamicMcpConfig, ...mcpConfig }
```

启用条件：
- `--chrome` 参数或 `angshengInChromeDefaultEnabled` 配置
- Chrome 扩展已安装
...
| `src/utils/angshengInChrome/common.ts` | Chrome MCP 保留名与工具名 |
