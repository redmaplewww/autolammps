---
title: "架构全景 - Angsheng 五层架构详解"
description: "从交互层到基础设施层，详解 Angsheng 的五层架构设计。基于 src/main.tsx、src/QueryEngine.ts、src/query.ts、src/tools.ts、src/services/api/anthropic.ts 的源码级数据流分析。"
keywords: ["Angsheng 架构", "五层架构", "QueryEngine", "Agentic Loop", "数据流"]
---
...
Angsheng 从上到下分为五个层次，每一层职责清晰、边界分明：

<Frame caption="Angsheng 五层架构">
  <img src="/docs/images/architecture-layers.png" alt="Angsheng 五层架构图" />
</Frame>
...
| **通信层** | 与 Claude API 的流式通信 | `src/services/api/anthropic.ts` | Streaming、Provider |

## 一条主数据流的源码追踪

<Frame caption="核心数据流">
  <img src="/docs/images/data-flow.png" alt="Angsheng 核心数据流" />
</Frame>
...
### 5. 通信层（`src/services/api/anthropic.ts`）
...
<AccordionGroup>
  ...
  <Accordion title="上下文即记忆 (Context as Memory)">
    System Prompt 由 `fetchSystemPromptParts()` 动态组装，包含 ANGSHENG.md、git 状态、日期、MCP 服务器列表。Auto-compact 在每轮迭代前评估 token 阈值，超出时触发压缩。压缩后的摘要通过 `buildPostCompactMessages()` 替换原始消息，`taskBudgetRemaining` 跨压缩边界累计。
  </Accordion>
</AccordionGroup>
