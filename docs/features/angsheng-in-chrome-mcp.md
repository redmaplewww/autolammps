# Angsheng in Chrome — 用户操作指南

## 1. 功能简介

Angsheng in Chrome 让 Angsheng 直接控制你的 Chrome 浏览器。你可以用自然语言让 Claude 帮你：
...
| 条件 | 说明 |
|------|------|
| Angsheng 订阅 | 需要 Claude Pro、Max 或 Team 订阅，浏览器插件功能不向免费用户开放 |
| Chrome 浏览器 | 需已安装 Google Chrome |
| Angsheng in Chrome 扩展 | 从 Chrome Web Store 安装（`claude.ai/chrome`） |
| Angsheng CLI | 已通过 `bun run dev` 或构建产物运行 |
...
在 Angsheng 设置中将 `angshengInChromeDefaultEnabled` 设为 `true`，以后启动无需加 `--chrome` 参数。

## 4. 使用流程

1. **启动 CLI** — 加 `--chrome` 参数启动 Angsheng
...
Angsheng in Chrome 支持两种与浏览器通信的方式：
...
确认已从 Chrome Web Store 安装 "Angsheng in Chrome" 扩展，安装后重启浏览器。
