...
### 说明
通过 WebSocket 远程控制 Angsheng 会话。支持自托管私有部署。
...
**PR**: #93 `feat: enable Angsheng in Chrome MCP with full browser control`
...
### 说明
Daemon 模式允许 Angsheng 作为后台长驻进程运行，管理多个 worker。
...
### 说明
同一台机器上的多个 Angsheng 实例通过 UDS（Unix Domain Socket / Windows Named Pipe）自动发现并协作。首个启动的实例成为 main，后续自动注册为 sub。
...
### 说明
执行 `.angsheng/workflows/` 目录下的用户定义工作流脚本。

### 使用

**创建工作流**：
```bash
mkdir -p .angsheng/workflows
cat > .angsheng/workflows/deploy.sh << 'EOF'
#!/bin/bash
echo "Running tests..."
bun test
echo "Building..."
...
echo "Deploying..."
EOF
chmod +x .angsheng/workflows/deploy.sh
```
