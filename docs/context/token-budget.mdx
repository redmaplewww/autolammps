...
description: "从源码角度揭示 Angsheng token 预算管理：200K 上下文窗口的动态计算、截断机制、缓存优化和自动压缩的完整链路。"
...
Angsheng 的默认上下文窗口为 200K tokens（`MODEL_CONTEXT_WINDOW_DEFAULT = 200_000`），但实际可用于对话的空间远小于此：

```
上下文窗口（200K）
├── 系统提示词（~15-25K，缓存后成本低）
├── 工具定义（~10-20K，含 MCP 工具）
├── 用户上下文（ANGSHENG.md、git status 等）
...
```typescript
// src/services/api/anthropic.ts — getMaxOutputTokensForModel()
