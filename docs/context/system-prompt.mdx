...
description: "深入解析 Angsheng 的 System Prompt 动态组装过程：缓存策略、分界标记、Section 注册表、ANGSHENG.md 多级合并，以及如何将零散上下文拼装为 API 可消费的缓存友好结构。"
keywords: ["System Prompt", "系统提示词", "动态组装", "ANGSHENG.md", "Prompt Cache", "缓存策略"]
---

## 从数组到 API 调用：System Prompt 的完整链路

System Prompt 在 Angsheng 中不是一段写死的文本，而是一个 **`string[]` 数组**（品牌类型 `SystemPrompt`，定义于 `src/utils/systemPromptType.ts:8`），经过组装、分块、缓存标记后发送给 API。
...
3. **`buildSystemPromptBlocks()`**（`src/services/api/anthropic.ts:3279`）—— 调用 `splitSysPromptPrefix()` 分块，为每个块附加 `cache_control`
...
```typescript
`You are Angsheng, Anthropic's official CLI for Claude.\n\nCWD: ${getCwd()}\nDate: ${getSessionStartDate()}`
```
...
`src/services/api/anthropic.ts:348` 生成的 `cache_control` 对象：
...
```typescript
return [`You are Angsheng, Anthropic's official CLI for Claude.\n\nCWD: ${getCwd()}\nDate: ${getSessionStartDate()}`]
```

## 上下文注入：System Context 与 User Context

System Prompt 数组本身不包含运行时上下文（git 状态、ANGSHENG.md 内容）。上下文通过两个独立的管道注入：
...
export const getUserContext = memoize(async () => {
  return {
    claudeMd,            // 合并后的 ANGSHENG.md 内容
    currentDate,         // "Today's date is YYYY-MM-DD."
  }
})
```

- **ANGSHENG.md 禁用条件**：`CLAUDE_CODE_DISABLE_CLAUDE_MDS` 环境变量，或 `--bare` 模式（除非通过 `--add-dir` 显式指定目录）
...
- **`cc_version`**：Angsheng 版本 + 指纹
- **`cc_entrypoint`**：入口点标识（REPL / SDK / pipe 等）
- **`cch=00000`**（NATIVE_CLIENT_ATTESTATION 启用时）：Bun 原生 HTTP 层在发送前将零替换为计算出的哈希值，服务器验证此 token 确认请求来自真实 Angsheng 客户端

Header 始终 `cacheScope: null`——它因版本和指纹不同而变化，不适合缓存。

## ANGSHENG.md：项目级知识注入

这是 Angsheng 最巧妙的设计之一。在项目根目录放一个 `ANGSHENG.md` 文件，就能让 AI "理解" 你的项目：
...
系统会自动发现并合并多级 ANGSHENG.md：

```
~/.angsheng/ANGSHENG.md              ← 用户全局（个人偏好）
  └── /project/ANGSHENG.md         ← 项目根目录（团队共享）
        └── /project/src/ANGSHENG.md  ← 子目录（模块特定）
```

加载逻辑在 `src/utils/memoryFiles.ts` 中的 `getMemoryPrompts()` 和 `getMemoryFiles()` 实现——从 CWD 向上遍历目录树，合并所有匹配的 ANGSHENG.md 文件内容。
...
这是 Angsheng 在 token 成本优化上的核心设计——一次典型的 System Prompt 约 20K+ tokens，通过缓存分块可以节省 30-50% 的输入 token 费用。

## 兼容层：OpenAI 与 Gemini

Angsheng 提供了 OpenAI 和 Gemini 协议的兼容层，允许使用非 Anthropic 端点。
...
详见 `docs/plans/openai-compatibility.md` 和 `ANGSHENG.md` 中的相关章节。
