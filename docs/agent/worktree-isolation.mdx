...
description: "揭秘 Angsheng 的 git worktree 隔离机制：子 Agent 如何获得独立工作空间，worktree 创建/销毁生命周期、路径命名规则和安全防护。"
...
Worktree 文件统一存放在仓库根目录下的 `.angsheng/worktrees/`：
...
4. createWorktreeForSession(sessionId, slug)
   ...
   └── 无 hook → git 原生路径：
       a. getOrCreateWorktree(repoRoot, slug)
          ...
          └── 新建：
              i.   mkdir .angsheng/worktrees/（recursive）
...
5. 更新进程状态：
   ...
   - clearMemoryFileCaches() → 重新加载 worktree 中的 ANGSHENG.md
  ↓
