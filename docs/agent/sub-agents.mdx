...
description: "从源码角度解析 Angsheng 子 Agent：AgentTool.call() 的完整执行链路、Fork 子进程的 Prompt Cache 共享、Worktree 隔离、工具池独立组装、以及结果回传的数据格式。"
...
runAgent()                                    ← 核心执行（runAgent.ts）
  ...
  │   ├── 消息流逐条 yield
  │   └── recordSidechainTranscript()         ← JSONL 持久化（~/.angsheng/projects/{project}/{session}/subagents/）
  ↓
...
| **claude-code-guide** | 继承父模型 | 受限 | 解答 Angsheng 使用问题 |

用户还可通过 `.angsheng/agents/` 目录或 settings 定义自定义 Agent，作用域优先级为：managed settings > CLI `--agents` > 项目级 `.angsheng/agents/` > 用户级 `~/.angsheng/agents/` > plugin。
