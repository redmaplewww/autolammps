...
function isPathAllowed(
  ...
  // and internal editable paths live under ~/.angsheng/ — matching the ordering in
...
function checkPathConstraintsForStatement(
  ...
  // Runtime writes /project/.angsheng/settings.json (Claude's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.angsheng`, validate subsequent statements with
  // cwd='./.angsheng'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, Angsheng may only access files in the allowed working directories for this session: ${dirListStr}.`
