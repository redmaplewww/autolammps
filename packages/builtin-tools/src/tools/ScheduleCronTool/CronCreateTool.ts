...
import { getSessionId, setScheduledTasksEnabled } from 'src/bootstrap/state.js'
...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .angsheng/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this Claude session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
...
export const CronCreateTool = buildTool({
  ...
  async call({ cron, prompt, recurring = true, durable = false }) {
    ...
    const id = await addCronTask(
      ...
      effectiveDurable ? getSessionId() : undefined,
    )
  ...
  mapToolResultToToolResultBlockParam(output, toolUseID) {
    const where = output.durable
      ? 'Persisted to .angsheng/scheduled_tasks.json'
