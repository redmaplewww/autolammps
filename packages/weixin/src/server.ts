...
function formatPermissionRequestMessage(
  ...
  return [
    'Agent Aura needs your approval.',
    '',
    ...
    if (!account) {
      return {
        content: [
          {
            type: 'text',
            text: 'WeChat not connected. Run `aura weixin login` first.',
          },
        ],
        isError: true,
      }
...
export async function runWeixinMcpServer(
  ...
  if (!account) {
    process.stderr.write(
      '[weixin] No account configured. Run `aura weixin login` to connect your WeChat account.\n',
    )
