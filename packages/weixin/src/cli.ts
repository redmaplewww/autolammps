...
function printUsage(): void {
  process.stdout.write(
    [
      ...
      '  aura weixin serve',
      '  aura weixin login',
      '  aura weixin login clear',
      '  aura weixin access pair <code>',
      '',
      'Session enablement:',
      '  aura --channels plugin:weixin@builtin',
    ].join('\n') + '\n',
  )
}
  ...
  if (existing) {
    process.stdout.write(
      [
        ...
        '',
        'Run `aura weixin login clear` to disconnect.',
        'Restart Agent Aura with:',
        '  aura --channels plugin:weixin@builtin',
      ].join('\n') + '\n',
    )
    return
