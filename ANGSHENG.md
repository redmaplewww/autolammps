...
This file provides guidance to Angsheng (claude.ai/code) when working with code in this repository.

## Project Overview

This is a **reverse-engineered / decompiled** version of Anthropic's official Angsheng CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced（见 Working with This Codebase 段的 tsc 要求）。
...
- **`src/services/api/anthropic.ts`** — Core API client. Builds request params (system prompt, messages, tools, betas), calls the Anthropic SDK streaming endpoint, and processes `BetaRawMessageStreamEvent` events.
...
| `packages/@ant/chrome-mcp-bridge/` | Chrome 浏览器控制（通过 `--chrome` 启用） |
...
| `packages/cc-knowledge/` | Angsheng 知识库（非 workspace 包） |
...
- **`src/services/acp/`** — ACP agent 实现，包含 `agent.ts`（AcpAgent 类）、`bridge.ts`（Angsheng ↔ ACP 桥接）、`permissions.ts`（权限处理）、`entry.ts`（入口）。
...
- **`src/context.ts`** — Builds system/user context for the API call (git status, date, ANGSHENG.md contents, memory files).
- **`src/utils/memoryFiles.ts`** — Discovers and loads ANGSHENG.md files from project hierarchy.
