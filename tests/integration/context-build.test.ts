...
import { stripHtmlComments, isMemoryFilePath, getLargeMemoryFiles } from "../../src/utils/memoryFiles";
...
// ─── ANGSHENG.md Integration with System Prompt ─────────────────────────

describe("Context build: ANGSHENG.md + system prompt integration", () => {
...
// ─── ANGSHENG.md Discovery with Real File System ───────────────────────

describe("Context build: ANGSHENG.md file system integration", () => {
  let tempDir: string;

  test("strips HTML comments from ANGSHENG.md content", () => {
  ...
  test("isMemoryFilePath correctly identifies ANGSHENG.md paths", () => {
    expect(isMemoryFilePath("/project/ANGSHENG.md")).toBe(true);
    expect(isMemoryFilePath("/project/ANGSHENG.local.md")).toBe(true);
    expect(isMemoryFilePath("/project/.angsheng/rules/file.md")).toBe(true);
...
describe("Context build: large memory file filtering", () => {
  ...
  test("getLargeMemoryFiles returns empty when all files are small", () => {
    const files = [
      { path: "/a/ANGSHENG.md", content: "small" },
      { path: "/b/ANGSHENG.md", content: "also small" },
    ];
